 ...
 * FakeSelect Tags
 * Author : SeokY
 * registered date : 2017-02-08
 */
fakeselect.initialize = function() { // 옵션설정
	fakeselect({
		targetclassname : '',
		// 2021-09-07 추가
		ignoreclassname : 'default_select',
		usemultiple : true,
		title : {
			classname : 'select',
...
function fakeselect(v) {
	...
	function rc(sel,v,f) {
		...
		if (occk) {
			...
			var ga = (";" == style.substring(style.length-1, style.length)) ? "" : ";" ;
			if ("option" != f) {
				style += ga + "width:"+sel.noww+"px";
			} else {
				var widths = $(sel).find("option").map(function(idx, el){
						return el.text.length * 15;
					}).get(),
					maxwidth = Math.max.apply(null, widths);
				if (sel.noww > maxwidth) maxwidth = sel.noww;
				style += ga + "width:"+maxwidth+"px";
			}
	...
	function create(sel) {
		...
		if (!sel.multiple) {//normal
			...
			ac.onclick = function() {
				...
				this.opt.style.left=this.opt.style.top='-9999em';
				...
				if (this.opt.scrobj) {
					if (sel.scroll) {
						...
						//this.opt.scrobj.style.height='170px';
				...
				var top=bcr.top+sct-document.documentElement.clientTop - 34;
				if($("body").hasClass("shopplus")){
					top=bcr.top+sct-document.documentElement.clientTop;
				}
